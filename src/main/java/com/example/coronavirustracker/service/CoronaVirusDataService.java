...
public class CoronaVirusDataService {
    ...
    public void fetchVirusData() throws IOException, InterruptedException {
        ...
        for (CSVRecord record : records) {
            ...
            int totalCasesTillToday = Integer.parseInt(record.get(record.size()-1));
            int totalCasesTillYesterday = Integer.parseInt(record.get(record.size()-2));


            locationStats.setLatestTotalCases(Integer.parseInt(record.get(record.size()-1)));
            locationStats.setIncreaseDCaseFromYesterday(totalCasesTillToday-totalCasesTillYesterday);
